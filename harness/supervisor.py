...
able to halt / reroute / escalate). For now it does what a supervisor
must do for the *verifier* story to work: own the run's root span, run
workers underneath it, and report the trace id so clients can fetch the
verdict. The supervisor is itself an agent, and its spans are verified
like any other — that is the paper's "verification of meta-agents"
angle.
"""
...
class Supervisor:
    ...
    def run(self, task: str, workers: list[Worker]) -> SupervisorResult:
        ...
        with self._tracer.start_as_current_span(
            ...
            # The trace id is generated by the SDK at span start;
            # captured here so the caller can fetch the verdict report.
